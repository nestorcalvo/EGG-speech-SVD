...
def praat_vuv(audio_filaname, resultsp, resultst, time_stepF0=0, minf0=75, maxf0=500, maxVUVPeriod=0.02, averageVUVPeriod=0.01):
    command=path_base+'/Praat.exe'+' --run '+path_base+'/vuv_praat.praat '
...
def get_formants(audio_filename,sizeframe=0.025,step=0.01):
    """Formants calculation. Requires PRAAT
    Input 
        :param audio_filename: Name of the audio file (or path+name)
        :param sizeframe: Window size in seconds (defaul: 0.25s)
        :param step: Window step size (defaut: 0.1s)
        
        Output:
        :returns: Formants.F1,F2,DF1,DF2,DDF1,DDF2
    """
#    if os.path.isdir('./temp')==False:
#        os.makedirs('./temp')
    temp_filename=path_base+'/tempFormants.txt'
    np.savetxt(temp_filename,np.zeros((3,3)))
    
    praat_formants(audio_filename, temp_filename,sizeframe,step)
...
#    DDF2=np.diff(F2, n=2)
#    
#    formants = np.vstack([F1nz[0:-2],F2nz[0:-2],DF1[0:-1],DF2[0:-1],DDF1,DDF2])
...
def praat_formants(audio_filename, results_filename,sizeframe,step, n_formants=5, max_formant=5500):
    ...
    Parameters:
    	...
    	time_stepF0: time step to compute the pitch, default value is 0 and	 				Praat will use 0.75 / minf0
    ...
    Returns:
    	Nothing
    """
    #Buscar praat para el sistema operativo correcto
    command = path_base+'/Praat.exe'+' --run '+path_base+'/FormantsPraat.praat '#Windows
#    command = path_base+'\Praat'+' --run '+path_base+'\FormantsPraat.praat '#Linux
    command+=audio_filename + ' '+results_filename+' '
    command+=str(n_formants)+' '+ str(max_formant) + ' '
    command+=str(float(sizeframe))+' '
...
def decodeF0(fileTxt,len_signal,fs,win_size, time_stepF0):
    """
    ...
    if len(pitch_data.shape)>1:
        ...
        if len(pitch_data)>0:
            ...
            time_voiced=0 # First datum is the time stamp
            pitch=0 # Second datum is the pitch value
            
    if len_signal>0:
    	n_frames=(len_signal-int(win_size*fs))/int(time_stepF0*fs)
    	t = np.linspace(0.0,len_signal/fs,int(n_frames))
...
def decodeFormants(fileTxt):
	...
	Returns:
		...
		F2: Numpy array containing the values for the second formant
	"""
	fid=open(fileTxt)
	datam=fid.read()
