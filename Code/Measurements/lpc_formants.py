...
def get_formants(sig,fs,meth='AC',nform=3,pre_emph=True,alpha=0.97):
    """
    ...
    if pre_emph==True:
        ...
        #focuse on the high frequency components
    ...
    try:
        ...
        formants = np.zeros(nform)
        # formants = formants.reshape(1,-1)
        lpc_env = np.zeros(512)
...
def premph(sig,alpha=0.97):
    """
    Pre-emphasis (highpass filtering at 50Hz) is recommendedin order to supress 
    high intensity harmonics in the low frequencies, which heavely affects the 
    formants across the spectrum.

    Parameters
    ----------
    sig : TYPE
        Speech signal (short-time)
    alpha : Flotat, optional
        Pre-emphasis coefficient. The default is 0.97.
...
def formant_post(formants):
    """
    Post-processing of the formants. Mainly used after computation 
    of the formants to reduce outliers
...
def smooth_curve(y,N=5):
    """
    Apply a moving average to a signal
